...
// Filename: IsiApi.h
//
// Description: This file contains function prototypes for the IzoT SDK Interoperable
...
/*
 * ******************************************************************************
 * SECTION: IzoT SDK ISI API
 * ******************************************************************************
 *
 * This section details the IzoT SDK ISI API functions.
 */
 ...
 */
ISI_EXTERNAL_FN const IsiApiError IsiStop(void);
 ...
 *
 * Parameters:
 * size - Size of this data structure, in bytes, including the size field
 *        (this can be used for versioning in a future release).
 * type - type of the ISI engine.
 * flags - option flags when starting the engine.  Values 0x01, 0x04 and 0x08 have no effect.
 *         All unrecognized values will be ignored.

 * connections - Size of the connection table, in records.
 * didLength - Size of the default primary domain Id, only 1, 3, or 6 are allowed.
 * pDid - The primary domain ID default value. Always 6 bytes, but only didLength bytes are valid.
 * repeatCount - The repeat count used with all network variable connections, where all
 ...
 */
ISI_EXTERNAL_FN const IsiApiError IsiStart(unsigned size, IsiType type, IsiFlags flags, unsigned connections, unsigned didLength, const LonByte* pDid, unsigned repeatCount);
 ...
 *
 * Remarks:
 * The IsiTick function must be called periodically at an approximate rate of 250 ms. 
 * The function takes care of all periodic housekeeping tasks and regularly scheduled   
 * messages.
 */
 ...
 *
 * On an IzoT Device Stack EX, the IsiReturnToFactoryDefaults function returns
 * to the caller. The node reset doesn't cause a physical reset on a typical
 * IzoT device, but cleans up pending transactions and fires the protocol stack's
 * reset event. Typical applications use this event to start the ISI engine, if
 * desired.
 */
 ...
 *
 * Returns:
 * True - If <IsiOpenEnrollment> has been called for the specified assembly and the enrollment has not yet
 *        timed out, been cancelled, or confirmed.
 * False - Otherwise
 *
 * Remarks:
 ...
 */
ISI_EXTERNAL_FN LonBool IsiIsBecomingHost(unsigned Assembly);
 ...
 */
ISI_EXTERNAL_FN LonBool IsiIsRunning(void);
 ...
 *
 * Remarks:
 * All devices in an ISI network periodically broadcast their status by sending out DRUM messages. To discover
 * devices, you can monitor these status messages.
 */
 ...
 * engine is running.
 */
ISI_EXTERNAL_FN void IsiUpdateUserInterface(IsiEvent Event, LonByte Parameter);
 ...
 * enable diagnostics in the <IsiStart> function and override the <IsiUpdateDiagnostics>
 * callback function. This callback is normally disabled and the default implementation
 * of <IsiUpdateDiagnostics> does nothing. The ISI engine calls this function with the
 * Event parameter set to one of the values defined for the IsiDiagnostic enumeration in
 * Appendix A. Some of these events carry a meaningful value in the Parameter argument, as detailed
 ...
 *
 */
ISI_EXTERNAL_FN void IsiUpdateDiagnostics(IsiDiagnostic Event, LonByte Parameter);
 ...
 * Offset - zero-based and relates to the selector number Offset that is used with the enrollment of this assembly
 * PreviousIndex - sets to ISI_NO_INDEX to get the index of the first Nv.  Otherwise, sets to previous network variable
 *         index to get the next one.
 *
 * Returns:
 *  The valid network variable index.
 *
 * Remarks:
 * This function must return at least one valid network variable index for each
 * assembly number returned by <IsiGetAssembly>. The *offset* parameter is zero-based
 * and relates to the selector number offset that is used with the enrollment of this assembly.
 * The function operates only when the ISI engine is running.
 */
ISI_EXTERNAL_FN unsigned IsiGetNvIndex(unsigned Assembly, unsigned Offset, unsigned PreviousIndex);
 ...
 * assemblies. This function operates only when the ISI engine is running.
 */
ISI_EXTERNAL_FN void IsiCreateCsmo(unsigned Assembly, IsiCsmoData* const pCsmoData);
 ...
 *
 * Remarks:
 * The default implementation returns ISI_DEFAULT_GROUP (128). This function is only required if the
 ...
 * Returns the number of the first and the next applicable assembly that can join the enrollment
 ...
 *            Specifies the previous assembly number to return the next applicable assembly following the previous one.
 *
 * Returns:
 *  The assembly number.
 *
 ...
 * The function operates only when the ISI engine is running.
 */
ISI_EXTERNAL_FN unsigned IsiGetAssembly(const IsiCsmoData* pCsmoData, LonBool Auto, unsigned Assembly);
 ...
 */
ISI_EXTERNAL_FN IsiApiError IsiInitiateAutoEnrollment(const IsiCsmoData* pCsmaData, unsigned Assembly);
 ...
 * The function operates whether the ISI engine is running or not.
 */
ISI_EXTERNAL_FN LonBool IsiIsConnected(unsigned Assembly);
 ...
 * The function operates whether the ISI engine is running or not.
 */
ISI_EXTERNAL_FN LonBool IsiIsAutomaticallyEnrolled(unsigned Assembly);
 ...
 *
 * Parameters:
 * NvIndex - network variable index.
 *
 * Returns:
 *  True - a heartbeat for the network variable with the global index NvIndex has been sent.
 *  False - otherwise
 *
 * Remarks:
 */
ISI_EXTERNAL_FN LonBool IsiQueryHeartbeat(unsigned NvIndex);
 ...
 *
 * Parameters:
 * NvIndex - network variable index for which the heart beat needs to be issued.
 *
 * Returns:
 ...
 */
ISI_EXTERNAL_FN LonBool IsiIssueHeartbeat(unsigned NvIndex);
 ...
 * this function allows an application to send a message at one of the periodic message slots.
 * The default implementation of <IsiCreatePeriodicMsg> does nothing but return FALSE.
 */
ISI_EXTERNAL_FN LonBool IsiCreatePeriodicMsg(void);
 ...
 *  append    - if it is set to true, it means to open file for writing at the end of the file
 *              (appending) without removing the EOF marker before writing new data to the file;
 *              creates the file first if it doesn't exist.
 *              if it is set to false, it means to open an empty trace file for writing.
 ...
 *
 */
ISI_EXTERNAL_FN void IsiSetTracefile(const char* pFilename, LonBool append);
 ...
 * ******************************************************************************
 *
 *  This section defines the prototypes for the ISI API controlled enrollement option
 *
 */
 ...
 *
 *  Parameters:
 *  pUniqueId - the unique ID of the device to send a request for controlled enrollment
 *  Assembly  - the assembly of the device to send a request for controlled enrollment
 *  command   - the requested operation for a controlled enrollment request contained in
 ...
 *
 *  Remarks:
 *  The host sends a control enrollment request message to a device to be connected.
 *  If the device supports controlled enrollment,
 *  it responds with control response (CTRP) message and the device itself will
 *  call appropriate control enrollement function based on the command specified.
 *
 */
ISI_EXTERNAL_FN IsiApiError IsiControlCommand(const LonUniqueId* pId, unsigned Assembly, IsiControl command);
 ...
 */
ISI_EXTERNAL_FN const IsiApiError IsiCreateCsmoRegistrar(IsiCreateCsmoFunction handler);
ISI_EXTERNAL_FN const IsiApiError IsiGetAssemblyRegistrar(IsiGetAssemblyFunction handler);
ISI_EXTERNAL_FN const IsiApiError IsiGetNvIndexRegistrar(IsiGetNvIndexFunction handler);
ISI_EXTERNAL_FN const IsiApiError IsiGetPrimaryGroupRegistrar(IsiGetPrimaryGroupFunction handler);
ISI_EXTERNAL_FN const IsiApiError IsiGetWidthRegistrar(IsiGetWidthFunction handler);
ISI_EXTERNAL_FN const IsiApiError IsiQueryHeartbeatRegistrar(IsiQueryHeartbeatFunction handler);
ISI_EXTERNAL_FN const IsiApiError IsiUpdateDiagnosticsRegistrar(IsiUpdateDiagnosticsFunction handler);
ISI_EXTERNAL_FN const IsiApiError IsiUpdateUserInterfaceRegistrar(IsiUpdateUserInterfaceFunction handler);
